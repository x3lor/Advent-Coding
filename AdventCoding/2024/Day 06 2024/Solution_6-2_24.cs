public class Solution_6_2_24 : ISolution
{
    public void run()
    {
    ...
    private class Grid {
        ...
        public int Start() {
            ...
            for (int obstacleY = 0; obstacleY < height; obstacleY++) {
                Console.Write($"\rProgress: {obstacleY} of {height-1}");
                for (int obstacleX = 0; obstacleX < width; obstacleX++) {
                    if (Test(obstacleX, obstacleY)) {
                        counter++;
                    }
                }
            }
        ...
        private bool Test(int obstacleX, int obstacleY) 
        {
            if ((obstacleX == xStart && obstacleY == yStart) || grid[obstacleY, obstacleX] == '#') 
            {
                return false;
            }
