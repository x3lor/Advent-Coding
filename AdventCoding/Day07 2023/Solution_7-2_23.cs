public class Solution_7_2_23 : ISolution
{
    ...
    public class Hand {
        ...
        private static int GetHandType(string cards) {
            ...
            var numberOfJokers = listOfCounts[0];
            listOfCounts.RemoveAt(0);

            listOfCounts = listOfCounts.OrderByDescending(x => x).ToList();

            if (listOfCounts[0]+numberOfJokers == 5) return 7;
            if (listOfCounts[0]+numberOfJokers == 4) return 6;
            if (listOfCounts[0]+numberOfJokers == 3 && listOfCounts[1] == 2) return 5;
            if (listOfCounts[0]+numberOfJokers == 3) return 4;
            if (listOfCounts[0]+numberOfJokers == 2 && listOfCounts[1] == 2) return 3;
            if (listOfCounts[0]+numberOfJokers == 2) return 2;
            return 1;
        }
    } 
}